...
MongoClient.connect(MONGO_URI)
  ...
  app.get(`/${STUDENT_ID}/profile/stats`, verifyToken, async (req, res) => {
    try {
      const userId = new ObjectId(req.user.id);
  
      // Fetch following count
      const followingData = await db.collection("follows").findOne({ userId });
      const followingCount = followingData?.following?.length || 0;
  
      // Fetch followers count
      const followersCount = await db
        .collection("follows")
        .countDocuments({ following: userId });
...
app.get(`/${STUDENT_ID}/users/search`, verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Fetch follow data for the logged-in user
    const followData = await db
      .collection("follows")
      .findOne({ userId: req.user.id });
    const following = followData ? followData.following.map((id) => id.toString()) : [];

    // Extract unique users with uploads and their follow status
    const usersWithUploads = contents.reduce((acc, content) => {
      ...
      if (existingUser) {
        ...
        acc.push({
          ...
          isFollowing: following.includes(content.userId.toString()),
          userId: content.userId, // Include user ID for follow/unfollow functionality
        });
      }
      return acc;
    }, []);
...
app.get(`/${STUDENT_ID}/following`, verifyToken, async (req, res) => {
  try {
    const followData = await db.collection("follows").findOne({ userId: req.user.id });

    if (!followData || !followData.following.length) {
      return res.status(200).json({ count: 0, following: [] });
    }

    const followingUsers = await db
      .collection("users")
      .find({ _id: { $in: followData.following } }, { projection: { username: 1 } })
      .toArray();

    res.status(200).json({
      count: followingUsers.length,
      following: followingUsers.map((user) => ({ id: user._id, username: user.username })),
    });
  } catch (err) {
